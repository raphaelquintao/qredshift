 ...
 *
 * Tanner Helland algorithm, optimized via polynomial regression against interpolated reference.
 * RMSE: 0.0152 vs interpolated reference
 *
...
RGB kelvin_to_rgb(double kelvin) {
	...
	if (kelvin == 6500) return color;

	if (temp <= 66) {
		/* warm side: 1000K - 6600K */
		r = 1.0;
		g = temp;
		g = 0.39310 * log(g) - 0.63184;

		if (temp <= 19) {
			b = 0.0;
		} else {
			b = temp - 10;
			b = 0.54520 * log(b) - 1.19625;
		}
	} else {
		/* cool side: 6600K - 25000K */
		r = temp - 60;
		r = 1.29294 * pow(r, -0.13750);
		g = temp - 60;
...
QGAMMA *calculate_gamma_ramp(int kelvin, double bright, double gamma, int ramp_size, int interpolation) {
	...
	const int apply_pow = (fabs(gamma - 1.0) > 1e-9);

	RGB temperature = interpolation ? kelvin_to_rgb_interpolated(kelvin) : kelvin_to_rgb(kelvin);

	QGAMMA *gamma_ramps = malloc(sizeof(QGAMMA) + 3 * ramp_size * sizeof(uint16_t));
	gamma_ramps->r = (uint16_t *)(gamma_ramps + 1);
	gamma_ramps->g = gamma_ramps->r + ramp_size;
	gamma_ramps->b = gamma_ramps->g + ramp_size;

	for (int i = 0; i < ramp_size; i++) {
		double value = (double)i / (double)ramp_size;
		double tr = value * temperature.r * bright;
		double tg = value * temperature.g * bright;
		double tb = value * temperature.b * bright;

		if (apply_pow) {
			gamma_ramps->r[i] = (uint16_t)(pow(tr, inv_gamma) * MAX);
			gamma_ramps->g[i] = (uint16_t)(pow(tg, inv_gamma) * MAX);
			gamma_ramps->b[i] = (uint16_t)(pow(tb, inv_gamma) * MAX);
		} else {
			gamma_ramps->r[i] = (uint16_t)(tr * MAX);
			gamma_ramps->g[i] = (uint16_t)(tg * MAX);
			gamma_ramps->b[i] = (uint16_t)(tb * MAX);
		}
	}
...
/*
 * Reconstructs (kelvin, brightness, gamma) from a live gamma ramp.
 *
 * Gamma is estimated from the power-law ratio of two ramp entries:
 *   entry[i] = pow(i/size * color * bright, 1/gamma) * MAX
 *   => gamma = log(mid/last) / log(entry[mid]/entry[last])
 * https://en.wikipedia.org/wiki/Gamma_correction
 *
 * Kelvin is found by brute-force nearest-neighbor search in RGB space
 * against kelvin_to_rgb().
 */
GammaParams reverse_gamma_ramp(const QGAMMA *ramp, int ramp_size, int interpolation) {
	...
	for (int k = 1000; k <= 25000; k += 100) {
		RGB c = interpolation ? kelvin_to_rgb_interpolated(k) : kelvin_to_rgb(k);
	...
	RGB temp_rgb = interpolation ? kelvin_to_rgb_interpolated(params.kelvin) : kelvin_to_rgb(params.kelvin);
